...
public class NotibaseApi(
     ...
     * for a different bundle, a key belonging to another app. So the app
     * reports its half and the answer is the whole picture.
     */
    public fun setupTest(report: Map<String, Any?>): List<SetupCheck> {
        val res = request("POST", "/v1/setup-test", report)
        val raw = res["checks"] as? List<*> ?: return emptyList()
        return raw.mapNotNull { item ->
            val o = item as? Map<*, *> ?: return@mapNotNull null
            val id = o["id"] as? String ?: return@mapNotNull null
            val level = o["level"] as? String ?: return@mapNotNull null
            val title = o["title"] as? String ?: return@mapNotNull null
            SetupCheck(id, level, title, o["detail"] as? String)
        }
    }
