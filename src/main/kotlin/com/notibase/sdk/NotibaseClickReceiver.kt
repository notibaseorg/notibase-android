...
public class NotibaseClickReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // The destination. Notibase.openUrl runs the app's own urlHandler
        // first, so a custom scheme routes in-app exactly as it does on the
        // system-tray path — one behaviour, not two that drift apart.
        // A destination that opened IS the tap's outcome — launching the app
        // on top of it would put the launcher over the page the person just
        // asked for. A destination that could not open falls through, so a
        // tap is never answered with nothing.
        val url = intent.getStringExtra(EXTRA_URL)
        if (url != null && Notibase.openUrl(context, url)) return

        val next = context.packageManager.getLaunchIntentForPackage(context.packageName)
